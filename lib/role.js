...
class Role {
    constructor(title,salary,department_name,direct_reportee){
            ...
            try{
                const [rows,fields] = await promisePool.query(
                 ...
                 const [rows1,fields1] = await promisePool.query(
                   `INSERT INTO role(title,salary,department_id,direct_reportee) values ("${this.title}",${this.salary},${myDepartmentId[0].id},${this.direct_reportee}) `); 
